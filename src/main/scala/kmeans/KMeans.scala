...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    val pToMeans = points.par.groupBy(p=> findClosest(p, means))

    means.par.map(mean=> mean -> pToMeans.getOrElse(mean, GenSeq())).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    /**
      ...
      *
      * -- the method takes and returns generic collections --
    ...
    oldMeans.par.map(mean=> findAverage(mean,classified(mean))) // map preserves the order
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    /**  Finally, you will implement convergence detection.
      * The convergence detection method takes a sequence of old means and the sequence of updated means,
      * and returns a boolean indicating if the algorithm converged or not. Given an eta parameter,
      * oldMeans and newMeans, it returns true if the algorithm converged, and false otherwise:
      **/

    val oldNewMeans = oldMeans zip newMeans

    oldNewMeans.forall{case (p1,p2)=>{p1.squareDistance(p2)<=eta}}
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    if (!converged(eta)(means,updatedMeans))
      kMeans(points, updatedMeans, eta) else updatedMeans // your implementation need to be tail recursive
  }
}
